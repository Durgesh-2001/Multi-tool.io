...
export const handleSubscription = async (req, res) => {
  try {
    const { plan } = req.body;
    if (!['Free', 'Micro', 'Weekly', 'Super', 'Pro', 'Pro+'].includes(plan)) {
    ...
    if (planLower === 'micro') {
      // TEST PLAN: 1 minute duration for testing subscription expiration
      endDate.setMinutes(endDate.getMinutes() + 1);
    } else if (planLower === 'weekly') {
...
export const createRazorpayOrder = async (req, res) => {
  try {
    ...
    if (plan === 'Free') {
      ...
      amount = 1; // Test plan - minimal amount
    } else if (plan === 'Weekly') {
      amount = 9;
    } else if (plan === 'Super') {
      amount = 39;
