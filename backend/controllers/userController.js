...
export const sendOTP = async (req, res) => {
  try {
    ...
    if (!accountSid || !authToken || !verifySid) {
      // Development mode - generate and store OTP
    ...
    try {
      ...
      // Update user's mobile in database for reference
      await User.findOneAndUpdate(
        { mobile: cleanMobile },
        { 
          mobile: cleanMobile,
          otpExpiry: new Date(Date.now() + 10 * 60000) // 10 minutes expiry (Twilio Verify default)
        },
        { upsert: true, new: true }
      );
  ...
  try {
    ...
    try {
      ...
      if (!user) {
        ...
        user = new User({
          mobile: cleanMobile,
          name: `User${cleanMobile.slice(-4)}`, // Default name
      ...
      // If Twilio fails, return appropriate error
      return res.status(400).json({
        success: false,
...
export const loginWithOTP = async (req, res) => {
  try {
    ...
    try {
      ...
      // Update OTP expiry for reference
